    ...
    for i in range(len(impact_order)):
        for j in range(len(probability_order)):
            fig.add_annotation(
                ...
                showarrow=False,
                font=dict(color="white" if matrix_data[i][j] > (max(map(max, matrix_data)) / 2) else "black", size=14, weight="bold")
            )
...
def create_bar_charts(df):
    ...
    # Impact distribution
    impact_counts = df['Impact / Consequence Rating'].value_counts().reindex(
    ...
    probability_counts = df['Probability / Likelihood Rating'].value_counts().reindex(
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            # Visualizations
            tab1, tab2, tab3, tab4 = st.tabs(["Heat Map", "Bubble Chart", "Bar Charts", "Risk Details"])
